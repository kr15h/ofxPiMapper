...
void TextureMappingMode::onKeyPressed(Application * app, ofKeyEventArgs & args){
	...
	switch(args.key){
	 ...
	 case OF_KEY_LEFT:
		 moveSelectedTexCoord(app, ofDefaultVec2(-moveStep, 0.0f));
		 break;
	 ...
	 case OF_KEY_UP:
		 moveSelectedTexCoord(app, ofDefaultVec2(0.0f, -moveStep));
		 break;
...
void TextureMappingMode::onMouseDragged(Application * app, ofMouseEventArgs & args){
	if(!_bTranslateCanvas){
		...
		if(_bCropAreaDrag){
			...
			ofDefaultVec2 d = ofDefaultVec2(distance.x, distance.y);
...
void TextureMappingMode::drawTexture(Application * app){
	if(app->getSurfaceManager()->getSelectedSurface() != 0){
		...
		ofSetColor(255, 255, 255, 255);
		app->getSurfaceManager()->getSelectedSurface()->drawTexture(ofDefaultVec2(0, 0));
...
void TextureMappingMode::moveSelectedTexCoord(Application * app, ofDefaultVec2 by){
	if(app->getSurfaceManager()->getSelectedSurface() != 0){
		int selectedTexCoord = Gui::instance()->getTextureEditorWidget().getSelectedTexCoord();
